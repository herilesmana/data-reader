...
class Reader implements DataReaderInterface
{
     ...
     */
    public function __construct($type, $trainingData, ConfigurationInterface $config = null)
    {
        ...
        if($type == 'file') {
            $this->data = $this->parseCsv($trainingData, $config);
        }
        
        if($type == 'array') {
            $this->attributes = $trainingData['attributes'];
            $this->data = $trainingData['data'];
        }
